...
class Condition < ActiveRecord::Base
  ...
  # end
  #
  def self.precipitation_range(floor)
    [floor..(floor+0.5)]
  end

  def self.temp_range(temp_floor)
    [temp_floor..temp_floor + 9]
  end

  def self.condition_trips(temp_floor)
    conditions = where(max_temperature_f: temp_range(temp_floor))
    conditions.joins(:trips).group(:date).count("id")
  end

  def self.average_number_of_rides_from_temp(temp_floor)
  ...
  def self.highest_number_of_rides_from_temp(temp_floor)
