...
RSpec.describe Condition do
  ...
  describe 'Class Methods' do
    describe '.highest_number_of_rides_from_temp' do
      it 'shows the highest number of rides from temp ranges' do
        expect(Condition.highest_number_of_rides_from_temp(0)).to eq(3)
      end
    end

    describe '.condition_trips_prec' do
      it 'shows the highest number of rides from precipitation ranges' do
        expect(Condition.highest_number_of_rides_from_prec(2)).to eq(3)
      end
    end

    describe '.condition_trips_wind' do
      it 'shows the highest number of rides from wind ranges' do
        expect(Condition.highest_number_of_rides_from_wind(4)).to eq(3)
      end
    end

    describe '.condition_trips_vis' do
      it 'shows the highest number of rides from vibility ranges' do
        expect(Condition.highest_number_of_rides_from_visiblity(4)).to eq(3)
      end
    end
  end
end
